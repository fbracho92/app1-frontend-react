...
// --- NUEVO: REPORTE LEGAL DE CARTERA DE CR脡DITO (MARCA BLANCA) ---
export const printLegalDebtReport = async (ReportService, bcvRate) => {
    try {
        ...
        doc.setTextColor(255, 255, 255);
        doc.setFont('helvetica', 'bold');
        doc.text("RELACI脫N ANAL脥TICA DE CUENTAS POR COBRAR", 14, 12);

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text("CONTROL DE CARTERA DE CR脡DITO Y VENCIMIENTOS", 14, 18);

        doc.setFontSize(9);
        doc.text(`RIF: ${tenantConfig.companyDocument}`, 14, 24);
        doc.text(`Raz贸n Social: ${tenantConfig.companyName}`, 14, 29);
        ...
        autoTable(doc, {
            startY: 40,
            head: [['CLIENTE / RAZ脫N SOCIAL', 'RIF/CI', 'N掳 FACT', 'EMISI脫N', 'VENCIMIENTO', 'D脥AS VENC.', `SALDO (${tenantConfig.secondaryCurrency})`, `SALDO (${tenantConfig.primaryCurrency})`]],
            body: debts.map(d => {
        ...
        doc.text("Base Legal: Normas Internacionales de Informaci贸n Financiera (NIC 21) y Normativa Nacional Vigente.", 14, finalY + 9);
...
// --- FUNCI脫N GENERAR REPORTE PDF (DISE脩O MODERNO: MARCA BLANCA) ---
export const exportReportToPDF = (analyticsData, reportDateRange) => {
    if (!analyticsData || !analyticsData.salesOverTime) {
        return Swal.fire('Sin datos', 'No hay informaci贸n para generar el reporte.', 'warning');
    }
    ...
    doc.text(`Periodo: ${new Date(reportDateRange.start).toLocaleDateString()} 鈥?${new Date(reportDateRange.end).toLocaleDateString()}`, 14, 38);
    ...
    doc.setFontSize(11);
    doc.setTextColor(...colors.darkText);
    doc.text("1. Evoluci贸n de Ventas Diarias", 14, finalY);
    finalY += 4;
    ...
    doc.setFontSize(11);
    doc.setTextColor(...colors.darkText);
    doc.text("2. Productos M谩s Vendidos (Top 5)", 14, finalY);
    finalY += 4;
    ...
    doc.setFontSize(11);
    doc.setTextColor(...colors.darkText);
    doc.text("3. Rendimiento por Categor铆a", 14, finalY);
    finalY += 4;

    autoTable(doc, {
        ...cleanTableStyles,
        startY: finalY,
        head: [['Categor铆a', 'Participaci贸n', 'Total (Ref)']],
