...
// ??? HELPER UX PRO: Sanitización extrema anti-mojibake para jsPDF
const sanitizePDF = (text) => {
    if (!text) return '';
    return text.toString()
        .normalize("NFD") // Separa las letras de los acentos
        .replace(/[\u0300-\u036f]/g, "") // Borra los acentos puros (Ej: é -> e)
        .replace(/[^a-zA-Z0-9\s.,()\-:;/%&#]/g, "") // Destruye CUALQUIER símbolo corrupto o "basura"
...
export const exportReportToPDF = (analyticsData, reportDateRange, tenantBrand = null) => {
    ...
    // ?? EXTRACCIóN BLINDADA CON SANITIZADOR
...
doc.text(`Periodo: ${dateStart} al ${dateEnd}`, 14, headerY);
    ...
    autoTable(doc, {
        ...
        body: (analyticsData.salesByCategory || []).map(row => {
            ...
            // ?? APLICANDO SANITIZADOR A LA CATEGORíA
